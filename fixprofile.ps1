...
"@
[System.IO.File]::WriteAllText("C:\Users\pc\Downloads\eduapp\frontend\src\pages\student\Settings.jsx", $s, [System.Text.Encoding]::UTF8)
Write-Host "Settings.jsx done" -ForegroundColor Green

# ── 2. Fix Messages to use real Avatar component ──
$m = [System.IO.File]::ReadAllText("C:\Users\pc\Downloads\eduapp\frontend\src\pages\shared\Messages.jsx")

# Add Avatar import after existing imports
if ($m -notmatch "import Avatar") {
  $m = $m -replace "(import toast from 'react-hot-toast')", "`$1`nimport Avatar from '../../components/common/Avatar'"
}

# Replace MiniAvatar usage with Avatar — pass full user object
$m = $m -replace '<MiniAvatar name=\{([^.}]+)\.name\}(\s+size=\{(\d+)\})?', '<Avatar user={$1} size={$3} clickable={true}'
$m = $m -replace '<MiniAvatar name=\{active\.name\}(\s+size=\{(\d+)\})?', '<Avatar user={active} size={$2} clickable={true}'
$m = $m -replace 'size=\{\}', 'size={36}'
...
Write-Host "Messages.jsx avatar fixed" -ForegroundColor Green

# ── 3. Make sure users API includes avatar_url in conversations ──
$r = [System.IO.File]::ReadAllText("C:\Users\pc\Downloads\eduapp\backend\src\routes\messages.js")
Write-Host "Messages route exists: $($r.Length -gt 0)" -ForegroundColor Cyan

Write-Host "`nAll done! Restart frontend to see changes." -ForegroundColor Green
