# ── 1. Fix shared Profile.jsx — works for ALL roles ──
$p = @"
      ...
      <div style={{ ...card, display:'flex', alignItems:'center', gap:'20px', flexWrap:'wrap' }}>
        ...
        <div>
          ...
          <span style={{ fontSize:'.72rem', fontWeight:700, padding:'2px 10px', borderRadius:'20px',
            background:`rgba(${profile.role==='admin'?'239,68,68':profile.role==='teacher'?'99,102,241':profile.role==='guardian'?'245,158,11':'16,185,129'},0.12)`,
...
"@
[System.IO.File]::WriteAllText("C:\Users\pc\Downloads\eduapp\frontend\src\pages\shared\Profile.jsx", $p, [System.Text.Encoding]::UTF8)
Write-Host "Profile.jsx done (all roles)" -ForegroundColor Green

# ── 2. Add Profile route to teacher and parent if missing ──
$app = [System.IO.File]::ReadAllText("C:\Users\pc\Downloads\eduapp\frontend\src\App.jsx")

# Add teacher settings route if missing
if ($app -notmatch "/teacher/settings") {
  $app = $app -replace '(<Route path="/teacher/profile")', '<Route path="/teacher/settings" element={<Profile />} />`n          $1'
  Write-Host "Teacher settings route added" -ForegroundColor Green
}
# Add parent settings route if missing
if ($app -notmatch "/parent/settings") {
  $app = $app -replace '(<Route path="/guardian/profile")', '<Route path="/parent/settings" element={<Profile />} />`n          $1'
  Write-Host "Parent settings route added" -ForegroundColor Green
}
# Add admin profile route if missing
if ($app -notmatch "/admin/profile") {
  $app = $app -replace '(<Route path="/admin/system-settings")', '<Route path="/admin/profile" element={<Profile />} />`n          $1'
  Write-Host "Admin profile route added" -ForegroundColor Green
}
[System.IO.File]::WriteAllText("C:\Users\pc\Downloads\eduapp\frontend\src\App.jsx", $app, [System.Text.Encoding]::UTF8)

# ── 3. Verify routes ──
Write-Host "`nProfile/settings routes in App.jsx:" -ForegroundColor Cyan
Select-String -Path "C:\Users\pc\Downloads\eduapp\frontend\src\App.jsx" -Pattern "profile|settings" | ForEach-Object { Write-Host "  $($_.Line.Trim())" }
