...
function FormulaCard({ item, color }) {
  ...
  return (
    <div onClick={() => setOpen(o => !o)} style={{
      background: open ? 'rgba(99,102,241,0.06)' : '#0d1117',
      border: '1px solid ' + (open ? '#6366f1' : '#1f2937'),
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '10px' }}>
        <div style={{ flex: 1 }}>
          ...
          <code style={{ background: color + '22', color: color, padding: '3px 10px', borderRadius: '6px', fontSize: '.82rem', fontFamily: 'monospace' }}>{item.formula}</code>
        </div>
        <span style={{ color: '#6b7280', fontSize: '.9rem', flexShrink: 0 }}>{open ? '-' : '+'}</span>
      </div>
      {open && (
        <div style={{ marginTop: '12px', paddingTop: '12px', borderTop: '1px solid #1f2937' }}>
          <p style={{ color: '#d1d5db', fontSize: '.85rem', lineHeight: 1.7, margin: '0 0 10px' }}>{item.explain}</p>
...
export default function MathKnowledge() {
  ...
  })).filter(g => !search || g.topics.length > 0)

  const totalFormulas = GRADES.reduce((sum, g) => sum + g.topics.reduce((s, t) => s + t.items.length, 0), 0)
...
console.log('Done! MathKnowledge fixed -- all formulas inline, no navigation, search added, more formulas included.')
