...
const card = { background:"var(--surface)", border:"1px solid var(--border)", borderRadius:"14px", padding:"16px 18px", marginBottom:"8px", position:"relative", transition:"opacity 0.2s" };
...
export default function Notifications() {
 ...
 <div>
 <h1 style={{ fontSize:"1.7rem", fontWeight:800, color:"var(--text)", margin:"0 0 4px", display:"flex", alignItems:"center", gap:"10px" }}>
 Notifications
 ...
 <p style={{ fontSize:".875rem", color:"var(--sub)", margin:0 }}>{unreadCount} unread {notes.length} total</p>
 </div>
 ...
 style={{ background: tab===key ? "rgba(99,102,241,0.15)" : "transparent", border: tab===key ? "1px solid rgba(99,102,241,0.3)" : "1px solid var(--border)", color: tab===key ? "#a5b4fc" : "var(--sub)", borderRadius:"10px", padding:"7px 16px", cursor:"pointer", fontWeight: tab===key ? 700 : 400, fontSize:".82rem" }}>
 {label}
 </button>
 ))}
 </div>
 ...
 <p style={{ color:"var(--text)", fontWeight:700, margin:"0 0 6px" }}>{tab==="unread" ? "All caught up!" : "No notifications yet"}</p>
 <p style={{ color:"var(--sub)", fontSize:".85rem", margin:0 }}>{tab==="unread" ? "Switch to 'All' to see past ones." : "Notifications will appear here."}</p>
 </div>
 ) : ordered.map(n => {
 ...
 style={{ ...card, borderLeft:`3px solid ${n.pinned?"#f59e0b":n.read?"var(--surface-hover)":color}`, cursor:n.read?"default":"pointer", opacity:n.read?0.88:1 }}>
 ...
 <p style={{ fontWeight:700, color:"var(--text)", margin:0, fontSize:".9rem" }}>{n.title}</p>
 ...
 <p style={{ color:"var(--sub)", fontSize:".82rem", margin:"0 0 5px", lineHeight:1.5 }}>{n.message}</p>
 ...
 {n.type === "link_request" && n.action_taken && (
 <p style={{ color:"var(--sub)", fontSize:".78rem", marginTop:"8px", fontStyle:"italic" }}>You already responded to this request.</p>
 )}
 <p style={{ color:"var(--sub)", fontSize:".7rem", margin:0 }}>{new Date(n.created_at).toLocaleString()}</p>
 </div>
 ...
 style={{ background: myReactions.includes(emoji)?"rgba(99,102,241,0.2)":"rgba(255,255,255,0.04)", border: myReactions.includes(emoji)?"1px solid rgba(99,102,241,0.4)":"1px solid var(--border)", borderRadius:"20px", padding:"3px 10px", cursor:"pointer", fontSize:".8rem", color:"#d1d5db", display:"flex", alignItems:"center", gap:"4px" }}>
 {emoji} <span style={{ fontSize:".72rem", color:"var(--sub)" }}>{users.length}</span>
 </button>
 ))}
 ...
 style={{ background:"rgba(255,255,255,0.04)", border:"1px solid var(--border)", borderRadius:"20px", padding:"3px 10px", cursor:"pointer", fontSize:".75rem", color:"var(--sub)" }}>
 + 
 </button>
 {openEmoji===n.id && (
 <div style={{ position:"absolute", bottom:"115%", left:0, background:"var(--surface-hover)", border:"1px solid var(--surface-hover)", borderRadius:"12px", padding:"8px", display:"flex", gap:"4px", flexWrap:"wrap", width:"176px", zIndex:100, boxShadow:"0 8px 32px rgba(0,0,0,0.6)" }}>
 {EMOJIS.map(e => (
 ...
 onMouseEnter={ev=>ev.target.style.background="var(--surface-hover)"}
 ...
 style={{ background: n.pinned ? "rgba(245,158,11,0.22)" : "rgba(255,255,255,0.07)", border: n.pinned ? "1px solid rgba(245,158,11,0.6)" : "1px solid var(--sub)", borderRadius:"8px", padding:"5px 12px", cursor:"pointer", fontSize:".75rem", fontWeight:700, color: n.pinned ? "#fbbf24" : "var(--sub)" }}>
 {n.pinned ? "Unpin" : "Pin"}
 </button>
 ...
 <div onClick={e => e.stopPropagation()} style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:"16px", padding:"24px", width:"100%", maxWidth:"480px", maxHeight:"90vh", overflowY:"auto" }}>
 <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"20px" }}>
 <h2 style={{ color:"var(--text)", fontWeight:800, margin:0, fontSize:"1.2rem" }}> Send Notification</h2>
 <button onClick={() => setShowCompose(false)} style={{ background:"rgba(255,255,255,0.05)", border:"1px solid var(--surface-hover)", borderRadius:"8px", width:"30px", height:"30px", cursor:"pointer", color:"var(--sub)", fontSize:"1rem" }}></button>
 </div>
 <div style={{ display:"flex", flexDirection:"column", gap:"12px" }}>
 <div>
 <label style={{ color:"var(--sub)", fontSize:".78rem", display:"block", marginBottom:"4px" }}>Title *</label>
 <input style={inp} value={compose.title} onChange={e=>setCompose(p=>({...p,title:e.target.value}))} placeholder="Notification title..." />
 </div>
 <div>
 <label style={{ color:"var(--sub)", fontSize:".78rem", display:"block", marginBottom:"4px" }}>Message *</label>
 ...
 <div>
 <label style={{ color:"var(--sub)", fontSize:".78rem", display:"block", marginBottom:"4px" }}>Type</label>
 ...
 <div>
 <label style={{ color:"var(--sub)", fontSize:".78rem", display:"block", marginBottom:"4px" }}>Send to</label>
 ...
 <div>
 <label style={{ color:"var(--sub)", fontSize:".78rem", display:"block", marginBottom:"4px" }}>User ID</label>
 ...
 style={{ background:sending?"var(--surface-hover)":"linear-gradient(135deg,#6366f1,#4f46e5)", color:sending?"var(--sub)":"#fff", border:"none", borderRadius:"10px", padding:"12px", fontWeight:700, fontSize:".9rem", cursor:sending?"default":"pointer", marginTop:"4px" }}>
