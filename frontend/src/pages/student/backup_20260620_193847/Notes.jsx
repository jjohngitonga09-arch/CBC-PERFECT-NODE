                ...
                {curriculumCards.map((card, i) => (
                  <div key={i} style={{ border:'1px solid var(--border,var(--border))', borderRadius:'12px',
                    ...
                    {grade.id === 'PP1' && subject.id === 'Language Activities' && card.title === 'Capital Letters A-Z' ? (
                      ...
                      <>
                         {card.title && card.title.includes('Assessed') ? (
                           <AssessmentChecklist content={card.content} />
                         ) : (
                         ...
                         {card.images && (
                           <div style={{ display:'flex', flexDirection:'column', gap:'12px', marginTop:'16px' }}>
                             {card.images.map((img, idx) => {
                               ...
                               const imgSrc = isObj ? img.src : img;
                               const caption = isObj ? img.caption : null;
                                const isPlaceholder = isObj && img.placeholder;
                               return (
                                 <div key={idx}>
                                    {isPlaceholder ? (
                                      <div style={{ border:'2.5px dashed #c7d2fe', borderRadius:10, padding:'28px 16px', textAlign:'center', background:'#f5f7ff' }}>
                                         <div style={{ fontSize:28, marginBottom:8 }}>📷</div>
                                        <p style={{ margin:0, fontWeight:700, color:'#4338ca', fontSize:14 }}>Upload image here</p>
                                        {caption && <p style={{ margin:'6px 0 0', fontSize:12.5, color:'#6b7280' }}>{caption}</p>}
                                      </div>
                                    ) : (
                                      <>
                                        <img src={imgSrc} alt={caption || (card.title + ' ' + (idx+1))}
                                          style={{ width:'100%', borderRadius:'10px', border:'1px solid var(--border,var(--surface-hover))' }} />
                                        {caption && <p style={{ fontSize:'.75rem', color:'var(--sub,#6b7280)', marginTop:4, textAlign:'center' }}>{caption}</p>}
                                      </>
                                    )}
                                 </div>
                               );
