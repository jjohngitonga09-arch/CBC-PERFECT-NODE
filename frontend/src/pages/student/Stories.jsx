...
function Modal({ story, genre, onClose, onNext, onPrev, hasNext, hasPrev }) {
 ...
 <div>
 <p style={{ fontWeight: 800, color: 'var(--text)', margin: '0 0 3px', fontSize: '1.1rem' }}>{story.title}</p>
 <p style={{ color: 'var(--text)', fontSize: '.78rem', margin: 0 }}>
 ...
 borderColor: tab === t ? genre.color : 'var(--sub)',
 background: tab === t ? genre.color + '22' : 'transparent',
 color: tab === t ? genre.color : 'var(--sub)',
 ...
 <p style={{ fontSize: '1.8rem', fontWeight: 900, color: 'var(--text)', margin: '0 0 6px' }}>
 {score}/{story.questions.length}
 </p>
 <p style={{ color: 'var(--text)', fontSize: '.875rem', margin: '0 0 24px' }}>
 ...
 <div key={i} style={{ marginBottom: '16px', padding: '16px', background: 'var(--bg)', borderRadius: '10px', border: '1px solid var(--border)' }}>
 <p style={{ fontWeight: 700, color: 'var(--text)', margin: '0 0 12px', fontSize: '.88rem', lineHeight: 1.5 }}>
 {i + 1}. {q.q}
 </p>
 ...
 borderColor: answers[i] === j ? genre.color : 'var(--sub)',
 background: answers[i] === j ? genre.color + '22' : 'var(--bg)',
 color: answers[i] === j ? genre.color : 'var(--sub)',
 ...
 background: answeredAll ? 'linear-gradient(135deg,' + genre.color + ',' + genre.color + 'bb)' : 'var(--surface-hover)',
 color: answeredAll ? '#fff' : 'var(--sub)',
...
function GenreSection({ genre, onSelect }) {
 ...
 <div style={{ flex: 1 }}>
 <h2 style={{ fontWeight: 900, color: 'var(--text)', margin: 0, fontSize: '1.4rem' }}>{genre.label}</h2>
 <p style={{ color: 'var(--sub)', fontSize: '.78rem', margin: 0 }}>{genre.desc}</p>
 </div>
 <span style={{
 ...
 <p style={{ fontWeight: 700, color: 'var(--text)', fontSize: '.9rem', margin: '0 0 6px', lineHeight: 1.4 }}>{s.title}</p>
 <p style={{ color: 'var(--sub)', fontSize: '.72rem', margin: 0 }}>{s.questions.length} quiz questions</p>
 </button>
 ))}
 </div>
 </div>
...
export default function Stories() {
 ...
 <h1 style={{ fontSize: '2.2rem', fontWeight: 900, color: 'var(--text)', margin: '0 0 8px' }}> Stories</h1>
