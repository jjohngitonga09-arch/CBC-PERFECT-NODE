...
function SubmitModal({ assignment, onClose, onSubmitted }) {
 ...
 style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '18px', width: '100%', maxWidth: '520px', padding: '20px', margin: '0 12px' }}
 >
 <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
 <h2 style={{ color: 'var(--text)', fontWeight: 700, fontSize: '1.1rem', margin: 0 }}>Submit: {assignment.title}</h2>
 <button onClick={onClose} style={{ background: 'none', border: 'none', color: 'var(--sub)', cursor: 'pointer', fontSize: '1.2rem' }}></button>
 </div>
 ...
 <p style={{ color: 'var(--text)', fontSize: '.85rem', margin: 0, lineHeight: 1.55 }}>{assignment.model_text || assignment.objective}</p>
 </div>
 )}

 {/* Mode toggle */}
 <div style={{ display: 'flex', gap: '6px', marginBottom: '14px', background: 'var(--bg)', borderRadius: '10px', padding: '4px' }}>
 ...
 rows={5}
 style={{ width: '100%', background: 'var(--bg)', border: '1px solid var(--surface-hover)', color: 'var(--text)', borderRadius: '10px', padding: '12px', fontSize: '.875rem', resize: 'vertical', boxSizing: 'border-box', outline: 'none', marginBottom: '14px' }}
 />
 ) : (
 ...
 style={{ border: '2px dashed var(--surface-hover)', borderRadius: '12px', padding: '32px 20px', textAlign: 'center', cursor: 'pointer' }}
 >
 <p style={{ fontSize: '2rem', margin: '0 0 6px' }}>=</p>
 <p style={{ color: 'var(--text)', fontWeight: 600, fontSize: '.875rem', margin: '0 0 4px' }}>Click to choose file</p>
 <p style={{ color: 'var(--sub)', fontSize: '.78rem', margin: 0 }}>PDF, DOC, DOCX, TXT Max 10 MB</p>
 </div>
 ) : (
 ...
 <button onClick={() => { setFile(null); if (fileRef.current) fileRef.current.value = '' }} style={{ background: 'none', border: 'none', color: 'var(--sub)', cursor: 'pointer' }}></button>
 </div>
 )}
...
export default function StudentAssignments() {
 ...
 <p style={{ fontWeight: 700, color: 'var(--text)', margin: 0, fontSize: '.95rem' }}>{a.title}</p>
 </div>
 <p style={{ fontSize: '.75rem', color: 'var(--sub)', margin: '0 0 4px' }}>
 Grade {a.grade} {a.subject}
 {a.due_date ? ` Due ${new Date(a.due_date).toLocaleDateString()}` : ''}
 </p>
 {a.objective && <p style={{ fontSize: '.82rem', color: 'var(--sub)', margin: 0 }}>{a.objective}</p>}
 </div>
 {isSubmitted ? (
 ...
 {a.feedback && <span style={{ color: 'var(--sub)', fontWeight: 400, fontSize: '.78rem', marginLeft: '8px' }}>"{a.feedback}"</span>}
 </p>
 )}
 </div>
 )
 ...
 <p style={{ fontSize: '.875rem', color: 'var(--sub)', margin: 0 }}>{pending.length} pending {done.length} submitted</p>
 </div>

 {assignments.length === 0 ? (
 <div style={{ ...card, textAlign: 'center', padding: '56px 20px' }}>
 <p style={{ fontSize: '3rem', margin: '0 0 12px' }}>=</p>
 <p style={{ fontWeight: 700, color: 'var(--text)', margin: '0 0 6px' }}>No assignments yet</p>
 <p style={{ color: 'var(--sub)', fontSize: '.875rem', margin: 0 }}>Your teacher hasn't sent any assignments yet.</p>
 </div>
 ) : (
 <>
 {pending.length > 0 && (
