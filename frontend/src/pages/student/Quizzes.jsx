...
export default function StudentQuizzes() {
 ...
 <h1 style={{fontSize:'1.7rem',fontWeight:800,color:'var(--text)',margin:'0 0 4px',letterSpacing:'-0.4px'}}>Quizzes</h1>
 <p style={{fontSize:'.875rem',color:'var(--sub)',margin:0}}>{quizzes.length} available</p>
 </div>

 {quizzes.length === 0 ? (
 <div style={{...card,textAlign:'center',padding:'56px 20px'}}>
 <p style={{fontSize:'3rem',margin:'0 0 12px'}}>=</p>
 <p style={{fontWeight:700,color:'var(--text)',margin:'0 0 6px'}}>No quizzes yet</p>
 <p style={{color:'var(--sub)',fontSize:'.875rem',margin:0}}>Your teacher will assign quizzes here.</p>
 </div>
 ) : (
 ...
 <p style={{fontWeight:700,color:'var(--text)',margin:'0 0 4px'}}>{q.title}</p>
 <p style={{fontSize:'.8rem',color:'var(--sub)',margin:'0 0 14px'}}>{q.question_count || '?'} questions {q.subject || 'General'}</p>
