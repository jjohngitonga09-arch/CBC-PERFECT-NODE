...
export default function GuardianChildren() {
 ...
 return (
 <div>
 <h1 style={{ fontSize: "1.7rem", fontWeight: 800, color: "var(--text)", margin: "0 0 4px" }}>My Children</h1>
 <p style={{ color: "var(--sub)", fontSize: ".875rem", margin: "0 0 24px" }}>
 ...
 <p style={{ color: "var(--text)", fontWeight: 700, margin: "0 0 6px" }}>No children linked yet</p>
 ...
 style={{ ...card, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "12px", padding: "14px 18px", cursor: "pointer", border: selected?.id === c.id ? "1px solid #6366f1" : "1px solid var(--border)" }}>
 <div>
 <p style={{ fontWeight: 700, color: "var(--text)", margin: "0 0 3px" }}>{c.name}</p>
 <p style={{ fontSize: ".75rem", color: "var(--sub)", margin: 0 }}>Grade {c.grade} {c.status}</p>
 </div>
 ...
 {selected && (
 <div style={card}>
 <h2 style={{ color: "var(--text)", fontWeight: 700, fontSize: "1rem", margin: "0 0 16px" }}>
 ...
 <div key={label} style={{ background: "var(--bg)", borderRadius: "12px", padding: "14px", textAlign: "center" }}>
 ...
 <div key={a.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "var(--bg)", borderRadius: "10px", padding: "10px 14px", flexWrap: "wrap", gap: "8px" }}>
 <div>
 <p style={{ color: "var(--text)", fontWeight: 600, margin: "0 0 2px", fontSize: ".875rem" }}>{a.title}</p>
 {a.due_date && <p style={{ color: "var(--sub)", fontSize: ".72rem", margin: 0 }}>Due {new Date(a.due_date).toLocaleDateString()}</p>}
 </div>
 ...
 ) : <p style={{ color: "var(--sub)" }}>Failed to load.</p>}
 </div>
 )}
 </div>
 );
