...
export default function PaymentHistory() {
 ...
 <h1 style={{ fontSize: "1.7rem", fontWeight: 800, color: "var(--text)", margin: "0 0 4px" }}>Payment History</h1>
 <p style={{ fontSize: ".875rem", color: "var(--sub)", margin: 0 }}>{payments.length} payments KES {total.toLocaleString()} confirmed</p>
 </div>

 {payments.length === 0 ? (
 <div style={{ ...card, textAlign: "center", padding: "48px 20px" }}>
 <p style={{ fontSize: "2.5rem", margin: "0 0 10px" }}></p>
 <p style={{ color: "var(--text)", fontWeight: 700, margin: "0 0 6px" }}>No payments yet</p>
 <p style={{ color: "var(--sub)", fontSize: ".85rem" }}>Your M-Pesa payment history will appear here.</p>
 </div>
 ) : (
 payments.map(p => {
 ...
 <div style={{ flex: 1 }}>
 <p style={{ fontWeight: 700, color: "var(--text)", margin: "0 0 3px" }}>{p.plan_name || p.plan_id} Plan</p>
 <p style={{ color: "var(--sub)", fontSize: ".8rem", margin: "0 0 3px" }}>
 KES {Number(p.amount || 0).toLocaleString()} {new Date(p.created_at).toLocaleDateString()}
 </p>
 {p.mpesa_ref && <p style={{ color: "var(--sub)", fontSize: ".72rem", margin: 0 }}>Ref: {p.mpesa_ref}</p>}
 {p.message && <p style={{ color: "var(--sub)", fontSize: ".78rem", margin: "4px 0 0", fontStyle: "italic" }}>"{p.message}"</p>}
 </div>
