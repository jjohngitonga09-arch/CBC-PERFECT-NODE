...
export default function Login() {
 ...
 const handleSubmit = async (e) => {
  ...
  try {
   ...
   setAuth({ token, role, userId: user.id, grade: user.grade || null, linkedStudentIds: user.linkedStudentIds || [] });
   setUser(user);
   ...
   <div className="page-wrap">
    ...
    <aside className="left-panel">
     ...
     <div style={{ display:"inline-flex", alignItems:"center", gap:6, background:"rgba(82,183,136,0.12)", border:"1px solid rgba(82,183,136,0.3)", borderRadius:20, padding:"4px 12px", fontSize:".71rem", color:"#74c69d", marginBottom:"1.6rem" }}>
      Aligned with KICD CBC Framework
     </div>
    ...
    {/* RIGHT PANEL — shake wraps only this */}
    <section className={"right-panel" + (shake ? " shake" : "")}>
     ...
     <form onSubmit={handleSubmit}>
      {mode === "username" ? (
       <div style={{ marginBottom:".9rem" }}>
        ...
        <div style={{ position:"relative" }}>
         ...
         <input ref={usernameRef} className="form-input" type="text" placeholder="Enter your username"
       ...
       <div style={{ marginBottom:".9rem" }}>
        ...
        <div style={{ display:"flex", gap:6 }}>
         ...
         <input className="form-input" style={{ paddingLeft:".9rem" }} type="tel" placeholder="7XX XXX XXX"
          value={form.phone} onChange={e => setForm({...form, phone: e.target.value.replace(/\D/g,"").slice(0,9)})}
          inputMode="numeric"
          onKeyDown={e => { if (e.key === "Enter") { e.preventDefault(); passwordRef.current?.focus(); } }} />
        </div>
       </div>
      ...
      {capsLock && (
       <div style={{ background:"rgba(245,158,11,0.1)", border:"1px solid rgba(245,158,11,0.3)",
        borderRadius:8, padding:"7px 12px", marginBottom:"1rem",
        fontSize:".78rem", color:"#fcd34d", display:"flex", alignItems:"center", gap:6 }}>
        &#x21EA; Caps Lock is ON — your password may be incorrect
       </div>
      )}
     ...
     <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:12, flexWrap:"wrap", marginTop:"1.1rem", paddingTop:".9rem", borderTop:"1px solid rgba(255,255,255,0.08)", fontSize:".68rem", color:"rgba(255,255,255,0.28)" }}>
      <span> TLS 1.3</span><span> CSRF Protected</span><span> 5-attempt lockout</span>
     </div>
