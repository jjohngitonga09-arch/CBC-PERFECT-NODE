...
export default function ParentSubscription() {
 ...
 <h1 style={{ fontSize:'1.8rem', fontWeight:800, color:'var(--text)', margin:'0 0 6px' }}>Subscription Plans</h1>
 <p style={{ color:'var(--sub)', margin:0, fontSize:'.9rem' }}>
 ...
 <p style={{ color:'var(--sub)', fontSize:'.83rem', margin:0 }}>Admin will review and activate the plan shortly.</p>
 </div>
 )}
 ...
 style={{ width:'100%', padding:'10px', borderRadius:'10px', border:'none', background:isCurrent?'rgba(255,255,255,.05)':plan.color, color:isCurrent?'var(--sub)':'#fff', fontWeight:700, cursor:isCurrent?'default':'pointer', fontSize:'.875rem' }}>
 ...
 <p style={{ color:'var(--sub)', fontSize:'.83rem', margin:'0 0 16px' }}>Send M-Pesa ref to admin for manual activation.</p>
 <input style={{ width:'100%', background:'var(--bg)', border:'1px solid var(--border)', color:'var(--text)', borderRadius:'10px', padding:'10px 12px', marginBottom:'10px', fontSize:'.85rem', boxSizing:'border-box' }}
 placeholder="M-Pesa confirmation code" value={mpesaRef} onChange={e=>setMpesaRef(e.target.value)} />
 <textarea style={{ width:'100%', background:'var(--bg)', border:'1px solid var(--border)', color:'var(--text)', borderRadius:'10px', padding:'10px 12px', marginBottom:'10px', fontSize:'.85rem', boxSizing:'border-box', minHeight:'70px', resize:'vertical' }}
 ...
 style={{ width:'100%', padding:'11px', background:mpesaRef?'#6366f1':'var(--surface-hover)', color:mpesaRef?'#fff':'var(--sub)', border:'none', borderRadius:'10px', fontWeight:700, cursor:mpesaRef?'pointer':'default' }}>
 ...
 <h3 style={{ color:'var(--text)', fontWeight:800, margin:'0 0 4px' }}>Pay with M-Pesa</h3>
 <p style={{ color:'var(--sub)', margin:'0 0 20px', fontSize:'.85rem' }}>{modal.name} KES {Number(modal.price).toLocaleString()}/{modal.period}</p>
 <input style={{ width:'100%', background:'var(--surface)', border:'1px solid var(--border)', color:'var(--text)', borderRadius:'10px', padding:'11px 14px', fontSize:'1rem', boxSizing:'border-box', marginBottom:'16px' }}
 ...
 style={{ flex:2, padding:'11px', background:paying?'var(--surface-hover)':'#4ade80', color:paying?'var(--sub)':'#000', border:'none', borderRadius:'10px', fontWeight:700, cursor:paying?'default':'pointer' }}>
