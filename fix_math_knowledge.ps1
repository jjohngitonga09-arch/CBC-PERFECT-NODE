# fix_math_knowledge.ps1
# Run from EDUAPP root: C:\Users\pc\OneDrive\Desktop\eduapp
# Command: powershell -ExecutionPolicy Bypass -File fix_math_knowledge.ps1

$filePath = "frontend\src\pages\student\MathKnowledge.jsx"
...
# ─── Splice the data into the file ─────────────────────────────────────────
$lines  = Get-Content $filePath
# Indices 4,5,6 (lines 5-7) = the three placeholder lines
$before = $lines[0..3]
$after  = $lines[7..($lines.Count - 1)]

$newContent = ($before -join "`r`n") + "`r`n" + $gradesData + "`r`n" + ($after -join "`r`n")

Set-Content $filePath $newContent -Encoding UTF8

Write-Host ""
Write-Host "Done! MathKnowledge.jsx has been updated with full Grade 1-9 GRADES data."
Write-Host "Restart your dev server (npm run dev) to see the changes."
Write-Host ""
Write-Host "Quick check - new line count:"
